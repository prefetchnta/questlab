...
namespace pr2 {
	...
	void PlateRecognation::segmentation_free_recognation(cv::Mat src, pr2::PlateInfo &plateinfo)
	{
		float score = 0;
		string text = "";
		Mat src1 = src.clone();
		...
		{
			if (maxidxs[i] < pr2::CH_PLATE_CODE.size() && (i == 0 || (maxidxs[i - 1] != maxidxs[i])))
			{
				text += pr2::CH_PLATE_CODE[maxidxs[i]];
				score += maxscore[i];
				charnum++;
			}
		}
