...
namespace pr2 {
	...
	void PlateDetection::Detectssd(cv::Mat img, std::vector<pr2::PlateInfo>  &plateInfos)
	{
		int cols = img.cols;
		int rows = img.rows;
		Mat in;
	...
	void DBDetection::DBDetect(cv::Mat InputImage,std::vector<pr2::PlateInfo> &plateInfos,int min_w,int max_w)
	{
		cv::Mat processImage;
