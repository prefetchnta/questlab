...
static void
generate_proposals_yolo5(const ncnn::Mat& anchors,
                         int stride,
...
static void
generate_proposals_yolo5_pnnx(const ncnn::Mat& anchors,
                              int stride,
                              const ncnn::Mat& feat_blob,
                              float prob_threshold,
                              std::vector<Object>& objects)
...
static void
detect_yolo5(ncnn::Net *yolo, const cv::Mat& img, std::vector<Object>& objects,
             const sNCNN_YOLO_Param *param)
{
    int width = img.cols;
    int height = img.rows;
    ...
    int max_stride = (param->yolo_version == 5) ? 32 : 64;
    ...
    // stride 8
    {
        ...
        if (param->yolo_version == 500)
            generate_proposals_yolo5_pnnx(anchors, 8, out, param->prob_threshold, objects8);
        else
            generate_proposals_yolo5(anchors, 8, in_pad, out, param->prob_threshold, objects8);
    ...
    // stride 16
    {
        ...
        if (param->yolo_version == 500)
            generate_proposals_yolo5_pnnx(anchors, 16, out, param->prob_threshold, objects16);
        else
            generate_proposals_yolo5(anchors, 16, in_pad, out, param->prob_threshold, objects16);
    ...
    // stride 32
    {
        ...
        if (param->yolo_version == 500)
            generate_proposals_yolo5_pnnx(anchors, 32, out, param->prob_threshold, objects32);
        else
            generate_proposals_yolo5(anchors, 32, in_pad, out, param->prob_threshold, objects32);
...
/**********/
static void
detect_yolo7(ncnn::Net *yolo, const cv::Mat& img, std::vector<Object>& objects,
             const sNCNN_YOLO_Param *param)
{
    int width = img.cols;
    int height = img.rows;
    ...
    int max_stride = (param->yolo_version == 7) ? 32 : 64;
