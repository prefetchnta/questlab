...
int CPlateLocate::colorSearch(const Mat &src, const Color r, Mat &out,
  ...
  //if (m_debug) {
  //  utils::imwrite("easypr/resources/image/tmp/color.jpg", src_threshold);
  //}
...
int CPlateLocate::sobelSecSearchPart(Mat &bound, Point2f refpoint,
  ...
  if (bFindLeftRightBound(tempBoundThread, posLeft, posRight)) {
    ...
    utils::imwrite("easypr/resources/image/tmp/repaireimg1.jpg", bound_threshold);
    ...
    utils::imwrite("easypr/resources/image/tmp/repaireimg2.jpg", bound_threshold);
  }
...
int CPlateLocate::sobelSecSearch(Mat &bound, Point2f refpoint,
  ...
  utils::imwrite("easypr/resources/image/tmp/sobelSecSearch.jpg", bound_threshold);
...
void deleteNotArea(Mat &inmat, Color color = UNKNOWN) {
  ...
  if (BLUE == plateType) {
    ...
    utils::imwrite("easypr/resources/image/tmp/inputgray2.jpg", img_threshold);
...
int CPlateLocate::sobelOperT(const Mat &in, Mat &out, int blurSize, int morphW,
  ...
  utils::imwrite("easypr/resources/image/tmp/grayblure.jpg", mat_gray);
  ...
  utils::imwrite("easypr/resources/image/tmp/graygrad.jpg", grad);

  Mat mat_threshold;
  double otsu_thresh_val =
      threshold(grad, mat_threshold, 0, 255, CV_THRESH_OTSU + CV_THRESH_BINARY);

  utils::imwrite("easypr/resources/image/tmp/grayBINARY.jpg", mat_threshold);

  Mat element = getStructuringElement(MORPH_RECT, Size(morphW, morphH));
  morphologyEx(mat_threshold, mat_threshold, MORPH_CLOSE, element);

  utils::imwrite("easypr/resources/image/tmp/phologyEx.jpg", mat_threshold);
