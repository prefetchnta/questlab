...
GetTableIndexFromLocaleName (
  __CR_IN__ const wchar_t*  localeName
    )
{
    LOCALENAMEINDEX* localeNamesIndex;
    int middle, testIndex, bottom = 0;
    int top = cntsof(LocaleNameToIndexTable) - 1;

    localeNamesIndex = (LOCALENAMEINDEX*)LocaleNameToIndexTable;
    while (bottom <= top)
    {
        middle = (bottom + top) / 2;
        testIndex = __wcsnicmp_ascii(localeName, localeNamesIndex[middle].name,
    ...
    )
{
    ...
    if ((index < 0) || (index >= cntsof(LcidToLocaleNameTable)))
        return (0);
