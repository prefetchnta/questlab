    ...
    )
{
    ...
    dstlen = qlz_compress_2(src, (char*)dst, srclen, state);
    mem_free(state);
    return (dstlen);
}
    ...
    )
{
    ...
    ssize = qlz_size_compressed_2((const char*)src);
    dsize = qlz_size_decompressed_2((const char*)src);
    ...
    dstlen = qlz_decompress_2((const char*)src, dst, state);
    mem_free(state);
    return (dstlen);
}
