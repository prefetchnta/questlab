...
　　一个试验性质的程序集。各个程序使用进程通讯的方式组合起来，实现一些有用的功能。项目 **GameViewer** 的后续版本，实现了一些游戏引擎。一个通用的数据分析逆向工具集。各种技术的大杂烩（评估的平台）。
...
- **QuestLAB (VS2010 SP1)**
<p>系统启动程序 (离线)。程序集入口点，在启动之前执行一些批处理命令很难实现的初始化操作。</p>
...
- **QstMount (VS2010 SP1)**
<p>文件资源服务器程序。负责封包文件的挂载，子文件的加载，彻底解决包中包无限嵌套的问题。</p>
...
<p>文本信息框显示程序。提供两个文本显示框，用于显示一些文字信息，如加载解析文件的信息。</p>
...
- **QstText (BCB6.0 UPD4)**
<p>文本浏览执行编辑器。实现文本渲染、脚本执行功能，<b>GameViewer</b> 文本浏览部分的加强版。</p>
...
- **ImportLib (XHTML + CSS)**
<p>导入脚本仓库 (离线)。基于静态网页的导入脚本列表，<b>GameViewer</b> 游戏导入脚本列表的加强版。</p>

##【插件介绍】

　　所有插件位于目录 **bin/plugin** 下。**c** 开头的插件为命令扩展插件，用于扩展应用程序的消息命令；**f** 开头的插件为图片滤镜插件，用于实现二维图片的变换功能；**z** 开头的插件为 **CrHack/FMTz** 引擎插件，专门负责加载各类输入的数据；**x** 开头的插件则为复合插件，可以集中包括所有类型的插件功能模块。
