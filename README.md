...
　　一个试验性质的程序集。各个程序使用进程通讯的方式组合起来，实现一些有用的功能。作为 **GameViewer** 的后续版本，实现了一些游戏引擎。一个通用的数据分析逆向工具集。各种技术的大杂烩。
...
　　程序间协作的基本手段：（１）使用网络通讯实现相互消息的通知。目前可以使用 **CMD / INI / XML** 形式的消息数据；（２）使用磁盘文件实现非实时数据的交互；（３）使用内存共享文件实现大量数据的交换。
...
- **QuestLAB (VC10.0 W32 SDK)**
<p>系统启动程序 (离线)。整个系统的入口点，在启动之前执行一些批处理命令很难实现的初始化操作。</p>

- **QstServ (VC10.0 W32 SDK)**
<p>消息广播服务器程序。负责把一个程序的消息广播给系统内其他程序，是进程间网络交互的核心。</p>

- **QstLibs (VC10.0 W32 SDK)**
<p>基本功能函数封装库。提供基本功能给系统内或其他外部开发语言使用（如 <b>VB / PB / Delphi</b> 等等）。</p>

- **QstMount (VC10.0 W32 SDK)**
<p>文件资源服务器程序。负责封包文件的挂载，封包内文件的加载管理，彻底解决包中包无限嵌套的问题。</p>

- **QstMenu (BCB6.0 VCL)**
<p>整个系统的菜单程序。作为系统的主菜单、大总管、<b>GUI</b> 外壳，负责各部件的调度与界面统一控制。</p>

- **QstCmdz (VC10.0 W32 SDK)**
<p>命令行消息发送程序。可以发送所有形式的消息，支持批量消息的发送，同时也是批命令脚本的执行工具。</p>

- **QstIndex (BCB6.0 VCL)**
<p>正整数索引控制程序。可以控制与播放一个正整数索引值，主要用来控制多帧图片文件的帧编号。</p>

- **QstInfoz (BCB6.0 VCL)**
<p>文本信息框显示程序。提供主次两个文本显示框，用于显示一些普通的文字信息，如加载解析文件的信息。</p>

- **QstTree (BCB6.0 VCL + Virtual Treeview)**
<p>文件的树形列表程序。提供导入文件的选择功能，文件加载的主要发起者，<b>GameViewer</b> 文件列表的加强版。</p>

- **QstView2D (VC10.0 W32 SDK)**
<p>软件二维内容渲染器。实现数据文件的渲染与图形图像算法的实验，<b>GameViewer 2D</b> 渲染部分的加强版。</p>
...
　　所有插件位于目录 **bin/plugin** 下。字母 **c** 开头的插件为命令扩展插件，用于扩展一些应用程序的消息命令；字母 **f** 开头的插件为图片滤镜插件，用于实现二维图片的变换功能；字母 **z** 开头的插件为 **CrHack/FMTz** 引擎插件，专门负责加载各类输入的数据；字母 **x** 开头的插件则为复合插件，可以集中包括除 **CrHack/FMTz** 引擎插件外的所有插件功能模块。
