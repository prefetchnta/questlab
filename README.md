...
　　所有插件位于目录 **bin/plugin** 下。字母 **c** 开头的插件为命令扩展插件，用于扩展应用程序的消息命令；**f** 开头的插件为图片滤镜插件，用于实现二维图片的变换功能；**z** 开头的插件为 **CrHack/FMTz** 引擎插件，专门负责解析各类输入数据；**x** 开头的为复合插件，可以集中包括所有类型的插件功能模块。
